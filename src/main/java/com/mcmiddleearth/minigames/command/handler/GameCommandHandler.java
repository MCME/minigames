    ...
    protected HelpfulLiteralBuilder createCommandTree(HelpfulLiteralBuilder helpfulLiteralBuilder) {
        helpfulLiteralBuilder
                ...
                .then(HelpfulLiteralBuilder.literal("ready")
                        ...
                        .requires(sender -> PluginData.hasPermission(sender,Permission.MANAGER) && PluginData.isManager(sender) && PluginData.isNotAnnounced(sender))
    ...
    private int doCommand(McmeCommandSender sender, String command, String... args){
        ...
        switch (command){
            ...
            case "create":
                ...
                switch (type){
                    case GEO_GUESSR:
                        game = new GeoGame((Player) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The geo game was created.");
                        break;
                    case HIDE_AND_SEEK:
                        game = new HideGame((Player) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The hide game was created.");
                        break;
                    case LORE_QUIZ:
                        game = new QuizGame((Player) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The quiz game was created.");
                        break;
                    case MANHUNT:
                        game = new ManhuntGame((Player) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The manhunt game was created.");
                        break;
                    case RACE:
                        game = new RaceGame((Player) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The race game was created.");
                        break;
                    case WEREWOLF:
                        game = new WerewolfGame((Player) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
            ...
            case "join":
                ...
                game.addPlayer((Player) ((MinigameCommandSender) sender).getCommandSender());
                break;
            case "kick":
                game = PluginData.getGame(sender);
                game.kickPlayer(MiniGamesPlugin.getInstance().getProxyServer().getPlayer(args[0]).orElse(null));
                break;
            case "leaderboard":
                sendNotImplementedYetMessage(sender);
                break;
            case "leave":
                game = PluginData.getGame(sender);
                game.removePlayer((Player) ((MinigameCommandSender) sender).getCommandSender());
                break;
            case "manager":
                sendNotImplementedYetMessage(sender);
                break;
            case "ready":
                ...
                if(game instanceof QuizGame){
                    ...
                    quizGame.addPlayer((Player) ((MinigameCommandSender) sender).getCommandSender());
                }
                break;
            case "restart":
                sendNotImplementedYetMessage(sender);
            ...
            case "winner":
                ...
                if(game instanceof QuizGame){
                    QuizGame quizgame = (QuizGame) game;
                    quizgame.QuizGameWinner(((Player) ((MinigameCommandSender) sender).getCommandSender()));
