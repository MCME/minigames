/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.mcmiddleearth.minigames.command;

import com.mcmiddleearth.minigames.data.PluginData;
import com.mcmiddleearth.minigames.game.AbstractGame;
import com.mcmiddleearth.minigames.game.GeoGuessrGame;
import com.mcmiddleearth.minigames.game.QuizGame;
import org.bukkit.command.CommandSender;
import org.bukkit.entity.Player;

/**
 *
 * @author Eriol_Eandur
 */
public class GameRestart extends AbstractGameCommand{
    
    public GameRestart(String... permissionNodes) {
        super(0, true, permissionNodes);
        cmdGroup = CmdGroup.LORE_QUIZ;
        setShortDescription(": Restarts a game.");
        setUsageDescription(": Restarts a game which is resetting all scores and marking all questions as not answered.");
    }
    
    @Override
    protected void execute(CommandSender cs, String... args) {
        AbstractGame game = getGame((Player)cs);
        if(game != null && isManager((Player)cs,game)){
            if(game instanceof GeoGuessrGame){
                GeoGuessrGame geogame = (GeoGuessrGame) game;
                geogame.restartGame();
            }else if(game instanceof QuizGame){
                QuizGame quizgame = (QuizGame) game;
                quizgame.resetQuestions();
            }
            sendResetGameMessage(cs);
        }
    }

    private void sendResetGameMessage(CommandSender cs) {
        PluginData.getMessageUtil().sendInfoMessage(cs, "You restarted the game.");
    }
    
}
