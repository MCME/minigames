...
import com.mcmiddleearth.minigames.core.command.argument.*;
import com.mcmiddleearth.minigames.core.game.*;
import com.mcmiddleearth.minigames.core.game.otherCommands.GameCheck;
...
import net.md_5.bungee.api.connection.McmeProxyPlayer;
    ...
    private int doCommand(McmeCommandSender sender, String command, String... args){
        ...
        switch (command){
            ...
            case "create":
                ...
                switch (type){
                    case GEO_GUESSR:
                        game = new GeoGame((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The geo game was created.");
                        break;
                    case HIDE_AND_SEEK:
                        game = new HideGame((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The hide game was created.");
                        break;
                    case LORE_QUIZ:
                        game = new QuizGame((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The quiz game was created.");
                        break;
                    case MANHUNT:
                        game = new ManhuntGame((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The manhunt game was created.");
                        break;
                    case RACE:
                        game = new RaceGame((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
                        PluginData.getMessageUtil().sendInfoMessage(sender,"The race game was created.");
                        break;
                    case WEREWOLF:
                        game = new WerewolfGame((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender(),args[1]);
                        PluginData.addGame(game);
            ...
            case "delete":
                ...
                switch(args[0]){
                    case "quiz":
                        ...
                        if(file.exists()){
                            AbstractGame.addConversation((McmeProxyPlayer) ((MinigameCommandSender)sender).getCommandSender());
            ...
            case "join":
                ...
                game.addPlayer((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender());
                break;
            case "kick":
                game = PluginData.getGame(sender);
            ...
            case "leave":
                game = PluginData.getGame(sender);
                game.removePlayer((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender());
                break;
            case "manager":
                sendNotImplementedYetMessage(sender);
                break;
            case "ready":
                ...
                if(game instanceof QuizGame){
                    ...
                    quizGame.addPlayer((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender());
                }
                break;
            case "restart":
                sendNotImplementedYetMessage(sender);
            ...
            case "winner":
                ...
                if(game instanceof QuizGame){
                    QuizGame quizgame = (QuizGame) game;
                    quizgame.QuizGameWinner(((McmeProxyPlayer) ((MinigameCommandSender) sender).getCommandSender()));
