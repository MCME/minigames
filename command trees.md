...
graph LR
    ...
    Rgame --> remove --> R11([#checkpoint])
    Rgame --> show --> R12([start, finish, #checkpoint])
    Rgame --> files --> R13([race, marker])
    
    Qgame --> create --> quiz --> Q1([name])
    Qgame --> question --> Q2([single, multi, free, number])
              question --> list --> Q3([#page])
              question --> remove --> Q4([#question])
    ...
    Qame --> loadquestions --> Q15([categories]) --> Q16([matchAll])
    Qgame --> clear
    Qgame --> random --> Q17([off,questions,choices])
    Qgame --> winner
    Qgame --> files --> quiz
    
    Ggame --> create --> geo --> G1([name])
    Ggame --> ready
